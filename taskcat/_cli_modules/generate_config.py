...
class GenerateConfig:
    ...
    def __init__(
        ...
        if not project_root_path.exists():
            raise TaskCatException(f"Project root path {project_root_path} does not exist")
        ConfigGenerator(main_template=main_template,
                        output_file=output_file,
                        project_root_path=str(project_root_path),
                        owner_email=user_email,
                        aws_region=aws_region, replace=replace).generate_config()
